...
class General(Frame):
    def __init__(self, master, frame_look={}, **look):
        ...
        self.time_entry.grid(row=2, column=1, sticky=W, padx=40)
        ...
        self.date_entry.grid(row=3, column=1, sticky=W, padx=40)
        self.date_update()

    def date_update(self):
        self.time2 = time.strftime('%H:%M:%S')
        self.dt2 = time.strftime('%b %d %Y')
        # if time string has changed, update it
        if self.time2 != self.time1:
            time1 = self.time2
            self.dt.set(self.dt2)
